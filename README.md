...
- Las **7 piezas estándar** (I, O, T, S, Z, J, L) con colores diferenciados, más la pieza reto **Tuerca**: un bloque 3×3 con hueco en el centro.
- **Bomba**: cada 5 líneas aparece una pieza especial 1×1 que al fijarse destruye un área de 3×3 a su alrededor (+10 pts × nivel por bloque destruido; los bloques de arriba no caen).
...
- **Menú de pausa** (`P` o `Esc`): reanudar, reiniciar sin recargar la página, ver la lista de controles y elegir el **nivel inicial** (1–10) para la próxima partida (se guarda en `localStorage`). Mientras el menú está abierto el juego ignora las teclas, y al volver se descartan las teclas que sigan pulsadas para evitar movimientos accidentales.
...
- Un menú de pausa (`#pause-menu`) con los botones Reanudar, Reiniciar, Ver controles y el selector de nivel inicial.
- Un overlay para el estado **GAME OVER**.
...
- **Modelo del tablero**: una matriz `ROWS × COLS` donde cada celda guarda `0` (vacía) o un índice de color (1–9, el 9 es la bomba) que identifica la pieza.
...
- **Nivel y velocidad**: la partida empieza en el nivel inicial elegido en el menú de pausa y el nivel sube cada 10 líneas (nunca por debajo del inicial); la velocidad de caída se calcula como `max(100, 1000 − (level − 1) × 90)` milisegundos.
